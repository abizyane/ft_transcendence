...
import { useRouter } from "next/router"; // To access URL params
...
const Friends = () => {
  ...
  const { friends, loading, error, fetchFriendsof } = useFriendsof({ id: parseInt(userId) });
  ...
  const listHeight = isFriendListAlone ? 'h-[230px] lg:h-[600px]' : 'h-[400px] lg:h-[800px]';
  ...
  return (
    <div className="w-full lg:max-w-[1200px] p-2 mb-24 lg:h-full">
      ...
      <div className={`bg-gray-800/65 rounded-xl border w-full p-4 border-violet-primary ${listHeight} grid grid-cols-1 lg:grid-cols-2 gap-4  overflow-y-auto no-scrollbar p-41`}>
        {friends?.map((friend, index) => (
          <div key={index} className="flex justify-center items-center bg-gray-700/70 h-[90px] w-full hover:bg-gray-600 transition-shadow border border-gray-600 rounded-lg p-4 shadow-lg hover:shadow-2xl">
            <div className="h-14 w-14 rounded-full overflow-hidden">
              <img src={friend.profile_pic_url} alt="friend" className="w-full h-full object-cover" />
            </div>
            ...
            <div className="ml-auto flex space-x-2 lg:space-x-4">
              <button aria-label="Chat" className="hover:text-blue-500 text-white transition-colors">
                <IoChatbubbleEllipsesSharp className="w-6 h-6 text-blue-600 hover:text-blue-900" />
              </button>
              <button aria-label="Invite" className="hover:text-red-500 text-white transition-colors">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent className="mr-14 lg:mr-32 bg-gray-800/60 border-violet-primary">
                  <DropdownMenuItem><span className="text-white">Unfriend</span></DropdownMenuItem>
