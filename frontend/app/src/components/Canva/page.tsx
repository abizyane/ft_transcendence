...
export default function Canvas ({socketRef, callback, scoreSetter , setWinner, setLooser, scores}){
    ...
    useEffect(()=>{
        ...
        if (canvas){
            ...
            if (!GameRef.current)
                GameRef.current = new Game_Front(canvas, {player_one: {posX:bluePosRef.current.x, posY: 3, width: 2, height:100, color: gameCustomization.user_paddle_color  }, player_two:{posX:redPosRef.current.x, posY: 3, width: 2, height:100, color: gameCustomization.opponent_paddle_color}, ball:{color: gameCustomization.ball_color}})
        }
    }, [])
